...
######################################################
    ...
    #####################################################
        ...
        ### get parameters #### 
        self.Kp = rospy.get_param('~Kp',10) #Proportional gain to the PID controller. 
        self.Ki = rospy.get_param('~Ki',10)  #Integral gain to the PID controller. 
        self.Kd = rospy.get_param('~Kd',0.001) #Derivative gain to the PID controller. 
        self.out_min = rospy.get_param('~out_min',-255) #Minimum output of the wheel_vel topic. 
        self.out_max = rospy.get_param('~out_max',255) #Maximum output of the wheel_vel topic. 
        self.rate = rospy.get_param('~rate',30) #he frequency at which to publish the wheel_vel topic (in Hz) 
        self.rolling_pts = rospy.get_param('~rolling_pts',2) #he velocity used in the PID calculations and published as wheel_vel is calculated from rolling_pts points. 
        self.timeout_ticks = rospy.get_param('~timeout_ticks',4) #If no wheel_vtarget messages are received in timeout_ticks ticks, the wheels are stopped. (I.E. timeout_ticks / rate seconds) 
        self.ticks_per_meter = rospy.get_param('ticks_meter', 14860) #The number of wheel rotary encoder ticks per meter. This is a global parameter because it will be shared with other nodes. 
        self.vel_threshold = rospy.get_param('~vel_threshold', 0.001)
        self.encoder_min = rospy.get_param('encoder_min', -2147483648) #The min and max value the encoder should output. Used to calculate odometry when the values wrap around. 
        ...
        rospy.Subscriber("wheel", Int64, self.wheelCallback) #sub remap r_lwheelwheep from twm to ->wheel
        rospy.Subscriber("wheel_vtarget", Float32, self.targetCallback)  #subscr l_rwheel_vtargeret from twm -> wheel_vtarget
        self.pub_motor = rospy.Publisher('motor_cmd',Float32,queue_size=10) #remap left_wheel_speed to kit
        self.pub_vel = rospy.Publisher('wheel_vel', Float32,queue_size=10) #remap lwheel_vtarget
